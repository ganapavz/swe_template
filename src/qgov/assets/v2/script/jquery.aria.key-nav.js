...
(function( $ ) {
	...
	initManageFocus = function() {
		var controlSelector = $(this).data('controlSelector'),
	...
	processKeyHandlers = function( containerEl, keyHandlers, evt ) {
		var keyCode,
			evtKeyCode = getKeyCode( evt )
		;
		if ( keyHandlers[ evtKeyCode ] !== undefined ) {
			$.each( keyHandlers, function( key ) {
				// key is a string
				if ( parseInt( key, 10 ) === evtKeyCode ) {
					// Send through the controlset element as 'this'
					// Send through the evt object so that evt.target can also be used
					keyHandlers[ key ].apply( containerEl.get( 0 ), [ evt ] );
				}
			});
		}
	};
	...
	$.fn.findNextControl = function (direction) {
		...
		// find next control
		var containerEl = $(this).closest('.'+$.AKN.conf.controlsetClass),	
				...
				switch ( direction ) {
					case $.AKN.DIRECTION_PREV:
						// navigation is circular, if there are no more elements before, wrap to the last element
							return ( currentControlIndex-1 >= 0 ) ? allControls.eq( currentControlIndex - 1 ) : allControls.eq( allControls.length - 1 );
					...
					case $.AKN.DIRECTION_LAST:
						// jump straight to the last item
							return allControls.eq( allControls.length - 1 );

					default:
					//case $.AKN.DIRECTION_NEXT:
						// navigation is circular, if there are no more elements on the end, wrap to the start
							return ( currentControlIndex + 1 < allControls.length ) ? allControls.eq( currentControlIndex + 1 ) : allControls.eq( 0 );
				}
	};
