...
if (typeof jQuery === 'undefined') {
	$.debug('Video player warning: Initialisation failed. jQuery is not available', 'error');
	...
	jQuery.qg.ovt.defaultOptions = {
		...
		videowidth    : '640',
		videoheight   : '360',
		// player.swf must be on same server as video page
		playerswf     : '/assets/v2/flash/player.swf',
		accessswf     : 'accessibility.swf', // TODO relative to html page (!?)
	...
	jQuery.qg.ovt.conf = {
		...
		controlsTemplate      :
			...
			'<li class="play"><a href="#" title="Play"><img src="' + qg.swe.paths.assets + 'images/video/displacement.png" alt="" class="for-css-replacement" /><span>|></span></a></li>' +
			'<li class="progress-bar"></li>' +
			'<li class="progress" title="Current position"></li>' +
			'<li class="duration" title="Video duration"></li>' +
			'<li class="volume"><a href="#" title="Volume"><img src="' + qg.swe.paths.assets + 'images/video/displacement.png" alt="" class="for-css-replacement" /><span>*)</span></a></li>' +
			'<li class="captions"><a href="#" title="Captions"><img src="' + qg.swe.paths.assets + 'images/video/displacement.png" alt="" class="for-css-replacement" /><span>CC</span></a></li>' +
			'<li class="audio-description"><a href="#" title="Audio Description"><img src="' + qg.swe.paths.assets + 'images/video/displacement.png" alt="" class="for-css-replacement" /><span>AD</span></a></li>' +
			// '<li class="fullscreen"><a href="#" title="Fullscreen"><img src="' + qg.swe.paths.assets + 'images/video/displacement.png" alt="" class="for-css-replacement" /><span>FS</span></a></li>'
			'</ul></div>'
	};
	...
	(function($, qg) { /* start closure */
		...
		$.fn.qgOvt = function (options) {
			...
			if (!DetectFlashVer(9, 0, 115)) {
				$.debug('Online video template was not initialised. The template requires Flash 9.0.115 or later.','info');
				return;
			}
		...
		initQGVideoTemplate = function (options) {
			...
			// Enforce that each video has a unique id (or throw error)
				...
				if (containerId == '' || $.inArray(containerId, $.qg.ovt.instances) !== -1) {
					$.debug('Video player warning: Initialisation failed. Each video template container must have a unique id', 'error');
					return false;
				}
		...
		parseVideoHTML = function (options) {
			...
			// Find video file. flv and mp4 only supported at this time
				...
				if (typeof options.sourceflv != 'undefined' && options.sourceflv != '') {
					...
					// parse duration from link
						if (videoLinkText != '') {
							...
							hours   = (hours   = videoLinkText.match(/(\d*)\s+hours?/i))   ? parseInt(hours[1],   10) : 0 ;
							minutes = (minutes = videoLinkText.match(/(\d*)\s+minutes?/i)) ? parseInt(minutes[1], 10) : 0 ;
							seconds = (seconds = videoLinkText.match(/(\d*)\s+seconds?/i)) ? parseInt(seconds[1], 10) : 0 ;
				...
				if (videofile == '') {
					$.debug ('Video player warning: Initialisation failed. No acceptable video file could be found (flv, mp4)', 'warn');
			...
			// find CC files
				...
				if (options.ccfile == '') {
					$.debug ('Video player warning: No captions file could be found (timed text xml)', 'warn');
				}
			...
			// find AD files
				...
				if (options.adfile == '') {
					$.debug ('Video player notice: No audio description file could be found (mp3)', 'notice');
				}
			...
			// find poster image
				...
				if (options.poster == '') {
					$.debug ('Video player notice: No poster image could be found (jpg, png)', 'notice');
				}
				...
				$(this).find('.'+$.qg.ovt.conf.optionsClass+' .download a').each(function(){
					...
					$(this)
						...
						.empty().append(
							...
							linkText.replace( /\s+/gm, ' ' ).replace(/(.*\(.*, )([a-z0-9]*?)( .*\))/i, '<span>$1</span>$2<span>$3</span>')
						)
		...
		initVidControls = function (vidControls) {
			...
			vidControls.find('.captions a').click(function() {
				var toggleState = $( this ).data('qgov-video-cc') || false;
				var vidContainer = getVidContainer(this);
				var playerID = vidContainer.data('playerID');
				var player = document.getElementById(playerID);

				// JW API
				player.hideCaptions( toggleState );
				
				// update text
				updateCCButton(vidContainer, ! toggleState );

				$( this ).data('qgov-video-cc', ! toggleState );
				return false;
			});
			
			// init audio description button
			updateADButton(vidControls, false);
			vidControls.find('.audio-description a').click(function() {
				var toggleState = $( this ).data('qgov-video-ad') || false;
				var vidContainer = getVidContainer(this);
				var playerID = vidContainer.data('playerID');
				var player = document.getElementById(playerID);

				// JW API
				player.muteAudio( toggleState );
				
				// update text
				updateADButton(vidContainer, ! toggleState );

				$( this ).data('qgov-video-ad', ! toggleState );
				return false;
			});
			
			// init transcript button
			updateTranscriptButton(vidControls, false);
			vidContainer.find('.transcript a').click(function() {
				var toggleState = $( this ).data('qgov-video-transcript') || false;
				var vidContainer = getVidContainer(this);
				var playerID = vidContainer.data('playerID');
				...
				$( this ).data('qgov-video-transcript', ! toggleState );
				return false;
			});
			...
			// insert volume slider code
				if ($.ui && typeof $.ui.slider != 'undefined') {
					...
					$.debug ('Video player warning: Volume slider could not be initialised. jQuery UI Slider was not found', 'warn');
				}
			...
			// init progress bar
				...
				// 'write' mode is for user interactions, 'read' mode is for displaying user feedback).
					if ($.ui && typeof $.ui.slider != 'undefined') {
						...
						$.debug ('Video player warning: Progress bar could not be initialised. jQuery UI Slider was not found', 'warn');
					}
	...
	/*     RESIZING     */
		...
		updateDimensions = function(container, availableDimensions) {
			...
			// Progressbar takes up whatever space is left
				...
				// 5px error margin for the sake of Firefox 3.6.6 and 18 for IE 6 & 7
					errorMargin = qg.oldIE && qg.oldIEversion < 8 ? 18 : 5 ;
		...
		resizePlayerToFitContainer = function(){
			...
			options = $(this).data('options');
				...
				// Otherwise IE6 incorrectly stretches parent container to fit video.
					if ( qg.oldIE && qg.oldIEversion < 7 ) { $(this).css('position','absolute'); }
						parentContainer = $(this).parent();
						availableDimensions = calculateDimensions(options.videowidth, options.videoheight, parentContainer.width());
					if ( qg.oldIE && qg.oldIEversion < 7 ) { $(this).css('position','static'); }
		...
		calculateDimensions = function(defaultWidth, defaultHeight, availableWidth, availableHeight, expandPastDefaults) {
			...
			/*
			$.debug('defaultWidth: '+defaultWidth);
			$.debug('defaultHeight: '+defaultHeight);
			$.debug('availableWidth: '+availableWidth);
			$.debug('availableHeight: '+availableHeight);
			$.debug('expandPastDefaults: '+expandPastDefaults);
			$.debug('width: '+width);
			$.debug('height: '+height);
			*/
