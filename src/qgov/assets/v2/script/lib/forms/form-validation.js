...
(function( $ ) {
	...
	submitValidationHandler = function( event ) {
		...
		if ( count > 0 ) {
			...
			//enable form sumission if submit button is disabled
			var disabledBtns = $('form[data-singlesubmit="true"]').find('input[type="submit"]:disabled');
			if(disabledBtns.length > 0){
				disabledBtns.each(function () {
					$(this).attr('disabled', false);
					$(this).siblings('i.disabled').remove();
				});
			}
