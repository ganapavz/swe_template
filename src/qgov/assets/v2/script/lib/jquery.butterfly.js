...
jQuery.butterfly.defaultOptions = {
	...
	closeButtonImage: qg.swe.paths.assets + 'images/skin/button-close.png', // Set to the path of your close button image
		...
		if ( ! isNaN( input )) {
			...
			// %, convert to pixels
				...
				if ( typeof centDimension !== 'undefined' ) {
					input = parseInt( input .substr( 0, input.length - 1 ).replace( /^\s + |\s + $/g, "" ), 10 );
			...
			// em, check for pxToEm and convert (or warn )
				if ( typeof Number.prototype.pxToEm !== 'undefined' ) {
					input = parseInt( input.substr( 0, input.length - 2 ).replace( /^\s + |\s + $/g, "" ), 10 );
					input = input.pxToEm({
					   reverse : true
					});
