 ...
 *   * 3.1.2: Fixed bug so that body classes are preserved on load.
 *   * 3.1.3: Fixed bug so that <style> elements don't trigger an error.
 *   * 3.1.4: Make sure IEs apply the appropriate stylesheet (redundant and a little hacky, but unfortunately necessary in some cases).
 */

/*global qg*/
(function($, qg) { /* start closure */

	var styleCount = 1; // Tracks the current number of <style> elements parsed.
	...
	processEachStylesheet = function (addClasses, addLinks) {
		...
		// remove old layout classes
			if (addClasses && typeof $('body').data('current-layout-names') != 'undefined') {
				$('body')
		...
		// check if any media query based stylesheets are now relevant
			for (var i = 0, len = $.transformer.styles.length; i < len; i++) {
				...
				// pxToEm returns incorrect values for IE6 at low resolutions
					if ( qg.oldIE && qg.oldIEversion < 7 && $.transformer.styles[i].minUnit == 'em' && currentWindowWidth < 200) {
...
//				console.log('min: '+minPx+'; '+'max: '+maxPx);
				...
				if (((!(minPx && minPx > currentWindowWidth) && !(maxPx && maxPx < currentWindowWidth)) || (!maxPx && !minPx))) {
					...
					if (addLinks) {
						// enable relevant stylesheets
							...
							currentLinkHref = currentLink[ 0 ].href;
							if ( /^https/.test( window.location.protocol ) && /^\//.test( currentLinkHref )) {
								// use absolute URL for https to suppress mixed content warning in IE
								currentLinkHref = 'https://' + window.location.host + currentLinkHref;
							}
							currentLink.after('<link type="'+currentLink.attr('type')+'" rel="'+currentLink.attr('rel')+'" media="'+$.transformer.styles[i].medium+'" href="'+currentLinkHref+'" class="current-media-query-stylesheet" />');
					}
		...
		// Make sure IEs apply the appropriate stylesheet (redundant and a little hacky, but unfortunately necessary in some cases).
		$('.current-media-query-stylesheet').each(function(){
			$(this).attr('href',$(this).attr('href'));
		});
	};
	...
	$.emToPx = function (emValue) {
		if (typeof Number.prototype.pxToEm == 'undefined') {
			$.debug('em value conversion failed. pxToEm could not be found. Returning em value unconverted.');
			return emValue;
		}
	...
	parseStylesheets = function () {
		...
		if (typeof $(this).attr('media') == 'undefined') {
			return; // only continue to parse those with a media attribute
		}
		...
		// layoutname = filename - .css
			...
			} else { // must be a <style>
				layoutname = 'style-'+styleCount++;
			}
